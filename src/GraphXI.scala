...
abstract class GraphXI[VD: ClassTag, ED: ClassTag] protected () extends Graph[VD, ED]{

}

object GraphXI {
  // Constructs a new graph from Edges
  def addEdge[VD: ClassTag, ED: ClassTag](
    graph: Graph[VD, ED],
    edge: RDD[Edge[ED]],
    defaultValue: VD
  ): Graph[VD, ED] = {
    ...
    fromEdges(newEdges, defaultValue)
  }

  // Adds a new Node to a graph
  def addNode[VD: ClassTag, ED: ClassTag](
    graph: Graph[VD, ED],
